...
struct Set(T)
{
    ...
    this(const T[] values ...) pure nothrow @safe
    {
        put(values);
    }
    ...
    this(const Set set) pure nothrow @safe
    {
        put(set);
    }
    ...
    void put(const T value) pure nothrow @trusted
    {
    ...
    void put(const T[] values ...) pure nothrow @safe
    {
        foreach (v; values)
            put(v);
    }
    ...
    void put(const Set set) pure nothrow @safe
    {
        foreach (key; set[])
            put(key);
    }
    ...
    bool opBinaryRight(string op)(const T rhs) const pure nothrow @nogc @trusted
        if (op == "in")
    {
    ...
    auto opSlice() const pure nothrow @nogc @safe
    {
